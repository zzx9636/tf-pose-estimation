...
from tf_pose.pose_dataset import get_dataflow_batch, DataFlowToQueue, CocoPose, get_dataflow_batch_rnn
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', type=str, default='/home/extra_disk/Git_Repo/tf-pose-estimation/models/train4/test')
    ...
    modelpath = logpath = './models/train4/'
    ...
    with tf.device(tf.DeviceSpec(device_type="CPU")):
        input_node = tf.placeholder(tf.float32, shape=(args.batchsize, output_h, output_w, 57), name='rnnInput')
        ...
        enqueuer = DataFlowToQueue(df, [input_node, input_pose, heatmap_node, vectmap_node], queue_size=128)
    ...
    with tf.device(tf.DeviceSpec(device_type="GPU")):
        ...
        # define optimizer
        step_per_epoch = 36000 // args.batchsize
    ...
    #sample_train = tf.placeholder(tf.float32, shape=(args.batchsize, 640, 640, 3))
    #sample_valid = tf.placeholder(tf.float32, shape=(10, 640, 640, 3))
    #train_img = tf.summary.image('training sample', sample_train, args.batchsize)
    #valid_img = tf.summary.image('validation sample', sample_valid, 10)
    ...
    with tf.Session(config=config) as sess:
        ...
        while True:
            ...
            if gs_num - last_gs_num2 >= 500:
                ...
                #print('saved')
                for images_test, pose_test, heatmaps, vectmaps in df_valid.get_data():
                    #print('val {}'.format(total_cnt))
