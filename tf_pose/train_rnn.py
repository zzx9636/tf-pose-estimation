    ...
    with tf.Session(config=config) as sess:
        ...
        while True:
            ...
            if gs_num - last_gs_num >= 500:
                ...
                # a.set_title('heatmap est')
                # tmp = np.amax(heatMat[:, :, :-1], axis=2)
                # plt.imshow(tmp, cmap=plt.cm.gray, alpha=0.5)
                # plt.colorbar()

                # a = fig.add_subplot(3, 2, 2)
                # a.set_title('heatmap gt')
                # tmp = np.amax(sample_heat[:, :, :-1], axis=2)
                # plt.imshow(tmp, cmap=plt.cm.gray, alpha=0.5)
                # plt.colorbar()
                ...
                # tmp2 = sample_paf.transpose((2, 0, 1))
                # tmp2_odd = np.amax(np.absolute(tmp2[::2, :, :]), axis=0)
                # tmp2_even = np.amax(np.absolute(tmp2[1::2, :, :]), axis=0)

                # a = fig.add_subplot(3, 2, 5)
                # a.set_title('Vectormap-x GT')
                # # plt.imshow(CocoPose.get_bgimg(inp, target_size=(vectmap.shape[1], vectmap.shape[0])), alpha=0.5)
                # plt.imshow(tmp2_odd, cmap=plt.cm.gray, alpha=0.5)
                # plt.colorbar()

                # a = fig.add_subplot(3, 2, 6)
                # a.set_title('Vectormap-y GT')
                # # plt.imshow(CocoPose.get_bgimg(inp, target_size=(vectmap.shape[1], vectmap.shape[0])), alpha=0.5)
                # plt.imshow(tmp2_even, cmap=plt.cm.gray, alpha=0.5)
                # plt.colorbar()
